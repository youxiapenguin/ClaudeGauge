...
Mac 上用菜单栏小程序 `claudegauge_mac.py`（顶部菜单栏显示「Claude 13%」，点开看三项 + 重置时间）。Mac 原生跑 `claude`，不需要 WSL。

```bash
pip3 install rumps pyte          # 依赖
python3 claudegauge_mac.py       # 运行（cg_probe.py 需在同一目录）
```

前提：macOS 已安装并登录过 Claude Code（`claude` 命令行），能联网。

## 刷新频率与额度消耗

- **每 10 分钟自动刷新一次**;取数失败时每 60 秒重试,恢复后回到 10 分钟节奏。倒计时("剩 Xh Ym")在两次刷新之间本地每分钟递减,不走网络。
- **刷新不消耗你的 Claude 额度**:`/usage` 只是向官方服务器查询用量数字,不向模型发送任何提示词,0 token。额度只在你真正和模型对话时才消耗。
